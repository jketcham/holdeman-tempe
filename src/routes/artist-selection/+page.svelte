<svelte:head>
  <title>Artist Selection – Holdeman Neighborhood Association</title>
  <meta
    name="description"
    content="Help select an artist for our 2024–2025 Neighborhood Grant Project! Check out the artists and submit your top choices."
  />
  <meta name="twitter:label1" content="Survey Deadline" />
  <meta name="twitter:data1" content="August 2nd" />
</svelte:head>

<main class="mx-auto mb-10 mt-4 flex max-w-2xl flex-col px-4 sm:px-0">
  <div class="flex flex-col pb-4 text-center">
    <h1 class="mb-2 font-display text-3xl font-bold uppercase">Artist Selection</h1>
    <h2 class="text-lg">Hardy Dr. Bike Lane Art</h2>
    <h2 class="text-lg text-neutral-500">Neighborhood Grant Project 2024-2025</h2>
  </div>

  <hr />

  <h1 class="c10" id="h.llnvmu62l4pw">
    <span class="text-2xl">Background</span>
  </h1>
  <p class="c1">
    <span>The Holdeman Neighborhood Association was awarded $20k in funding for our </span><span
      class="c4"
      ><a
        class="c0"
        target="_blank"
        rel="noreferer nofollow"
        href="https://www.tempe.gov/government/city-manager-s-office/neighborhood-services/neighborhood-grant-program"
        >Maryanne Corder Neighborhood Grant</a
      ></span
    ><span class="c7"
      >&nbsp;project for the 2024&ndash;2025 cycle! That means we can move forward with our project
      for completion with a deadline of June 2025.</span
    >
  </p>
  <h1 class="c10" id="h.3se90ozbrcez">
    <span class="text-2xl">Project</span>
  </h1>
  <p class="c1">
    <span>We&rsquo;ll be completing an </span><span class="c4"
      ><a
        class="c0"
        target="_blank"
        rel="noreferer nofollow"
        href="https://www.tempe.gov/government/transportation-and-sustainability/transportation/streetscape-projects/adaptive-streets"
        >Adaptive Streets</a
      ></span
    ><span class="c7"
      >&nbsp;project to paint art in the bike lanes along Hardy Dr. around 17th Pl in order to
      beautify the roadway, serve a placemaking function to welcome people to the area, and highlight
      the presence of the bike lane and sidewalk to motorists with the hope of slowing them down.</span
    >
  </p>
  <p class="c1 c9"><span class="c7"></span></p>

  <figure>
    <div class="grid grid-cols-1 justify-center sm:grid-cols-2">
      <picture>
        <source srcset="images/image3.webp" type="image/webp" />
        <img
          alt="Partially completed bike lane art."
          loading="lazy"
          src="images/image3.jpg"
          width="570"
          height="760"
        />
      </picture>
      <picture>
        <source srcset="images/image1.webp" type="image/webp" />
        <img
          alt="Bike lane art being painted by volunteers."
          loading="lazy"
          src="images/image1.jpg"
          width="570"
          height="760"
        />
      </picture>
    </div>
    <figcaption class="mt-1.5 text-center text-sm">
      Bike lane art along College Ave. in Broadmor
    </figcaption>
  </figure>

  <p class="c1 c9"><span class="c7"></span></p>
  <p class="c1">
    <span class="c7"
      >A similar project was completed by the Broadmor Neighborhood Association during the
      2023&ndash;2024 grant cycle along College Ave between Palmcroft Dr and Concorda Dr (photos
      above).</span
    >
  </p>
  <h1 class="c10" id="h.i8r9qp2oiw7z">
    <span class="text-2xl">Next</span>
  </h1>
  <p class="c1">
    <span class="c7"
      >We&rsquo;ll vote on the artist we want to work with to complete the project. We had many
      wonderful and talented artists respond to our request for proposals, so choosing won&rsquo;t be
      easy. That&rsquo;s where you come in!</span
    >
  </p>
  <p class="c1 c9"><span class="c7"></span></p>
  <p class="c1">
    <span class="c7"
      >Please review the artists below, and complete the survey to indicate your top two choices.</span
    >
  </p>
  <p class="c1 c9"><span class="c7"></span></p>
  <p class="c17 mt-4">
    <a class="c0" href="https://forms.gle/r56k6qCQSpYEhcix7?utm_source=holdeman_na">
      <button class="rounded bg-holdeman-400 px-6 py-4 font-bold transition hover:bg-holdeman-500">
        Take the artist selection survey here
      </button>
    </a>
  </p>

  <div class="mt-4 text-center italic">
    Survey submissions must be received by
    <strong class="inline-block">August 2nd!</strong>
  </div>

  <hr style="page-break-before: always; display: none" />

  <div class="mb-4 mt-8 flex items-baseline space-x-2" id="h.g3uvqgdlj0fr">
    <h2 class="text-2xl">Artists</h2>
    <span class="c11">Listed alphabetically</span>
  </div>

  <section class="flex flex-col space-y-6">
    <div>
      <h2 class="" id="h.l46pxmea0jhe">
        <span class="text-xl">Jake Early</span>
      </h2>
      <p class="c1"></p>
      <div class="mt-2 flex space-x-4">
        <a
          href="https://www.jakeearly.com/?utm_source=holdeman_na"
          target="_blank"
          rel="noreferer nofollow"
        >
          <button class="rounded bg-neutral-300 px-4 py-2 font-bold transition hover:bg-neutral-400">
            Artist Website
          </button>
        </a>
        <a
          href="https://drive.google.com/file/d/1M4Hb4Z1cMrwYHMGdkVuY1nV4KDvFh-Ez/view?utm_source=holdeman_na"
          target="_blank"
          rel="noreferer nofollow"
        >
          <button class="rounded bg-neutral-300 px-4 py-2 font-bold transition hover:bg-neutral-400">
            Artist Proposal
          </button>
        </a>
      </div>
    </div>

    <div>
      <h2 class="" id="h.nip9xhszsh5q">
        <span class="text-xl">Lauren Lee</span>
      </h2>
      <div class="mt-2 flex space-x-4">
        <a
          href="https://www.laurenleefineart.com/?utm_source=holdeman_na"
          target="_blank"
          rel="noreferer nofollow"
        >
          <button class="rounded bg-neutral-300 px-4 py-2 font-bold transition hover:bg-neutral-400">
            Artist Website
          </button>
        </a>
        <a
          href="https://drive.google.com/file/d/1CuxqrejYeP-RJwvE-dav-8nX2GIo8ksV/view?utm_source=holdeman_na"
          target="_blank"
          rel="noreferer nofollow"
        >
          <button class="rounded bg-neutral-300 px-4 py-2 font-bold transition hover:bg-neutral-400">
            Artist Proposal
          </button>
        </a>
      </div>
    </div>

    <div>
      <h2 class="" id="h.ihs3z2eqhtup">
        <span class="text-xl">Kyllan Maney</span>
      </h2>
      <div class="mt-2 flex space-x-4">
        <a
          href="https://www.kyllanmaney.com/?utm_source=holdeman_na"
          target="_blank"
          rel="noreferer nofollow"
        >
          <button class="rounded bg-neutral-300 px-4 py-2 font-bold transition hover:bg-neutral-400">
            Artist Website
          </button>
        </a>
        <a
          href="https://drive.google.com/file/d/1LnjLVG5rnm1-ME90dGyXn98jHXysEc0F/view?utm_source=holdeman_na"
          target="_blank"
          rel="noreferer nofollow"
        >
          <button class="rounded bg-neutral-300 px-4 py-2 font-bold transition hover:bg-neutral-400">
            Artist Proposal
          </button>
        </a>
      </div>
    </div>

    <div>
      <h2 class="" id="h.aytca62n5ck2">
        <span class="text-xl">Nicole Poppell</span>
      </h2>
      <div class="mt-2 flex space-x-4">
        <a
          href="https://www.nnuzzo.com/?utm_source=holdeman_na"
          target="_blank"
          rel="noreferer nofollow"
        >
          <button class="rounded bg-neutral-300 px-4 py-2 font-bold transition hover:bg-neutral-400">
            Artist Website
          </button>
        </a>
        <a
          href="https://drive.google.com/file/d/13P75cB6gAsWuIUgWl8oWpmU3j0TFF9Af/view?utm_source=holdeman_na"
          target="_blank"
          rel="noreferer nofollow"
        >
          <button class="rounded bg-neutral-300 px-4 py-2 font-bold transition hover:bg-neutral-400">
            Artist Proposal
          </button>
        </a>
      </div>
    </div>

    <div>
      <h2 class="" id="h.14riemhxw0e7">
        <span class="text-xl">Rebecca Ross</span>
      </h2>
      <div class="mt-2 flex space-x-4">
        <a
          href="https://www.rebeccarossstudio.com/?utm_source=holdeman_na"
          target="_blank"
          rel="noreferer nofollow"
        >
          <button class="rounded bg-neutral-300 px-4 py-2 font-bold transition hover:bg-neutral-400">
            Artist Website
          </button>
        </a>
        <a
          href="https://drive.google.com/file/d/1Pw4ufSIuqJl5XiGoE3YWGHCO__OBKUgI/view?utm_source=holdeman_na"
          target="_blank"
          rel="noreferer nofollow"
        >
          <button class="rounded bg-neutral-300 px-4 py-2 font-bold transition hover:bg-neutral-400">
            Artist Proposal
          </button>
        </a>
      </div>
    </div>
  </section>

  <div class="mb-10 mt-8">
    <h2 class="text-2xl">Updates</h2>

    <p class="my-4">
      Sign up to receive our email newsletter and stay up to date on the project and neighborhood
      news.
    </p>
    <div class="flex justify-center">
      <a class="c0" href="https://holdemantempe.org/newsletter">
        <button class="rounded bg-holdeman-400 px-6 py-4 font-bold transition hover:bg-holdeman-500">
          Newsletter signup
        </button>
      </a>
    </div>
  </div>
</main>

<style type="text/css">
  .c10 {
    padding-top: 20pt;
    padding-bottom: 6pt;
    page-break-after: avoid;
    orphans: 2;
    widows: 2;
    text-align: left;
  }
  .c7 {
    color: #000000;
    font-weight: 400;
    text-decoration: none;
    vertical-align: baseline;
    font-style: normal;
  }
  .c11 {
    color: #000000;
    font-weight: 400;
    text-decoration: none;
    vertical-align: baseline;
    font-size: 11pt;
    font-style: italic;
  }
  .c17 {
    padding-top: 0pt;
    padding-bottom: 0pt;
    orphans: 2;
    widows: 2;
    text-align: center;
  }
  .c1 {
    padding-top: 0pt;
    padding-bottom: 0pt;
    orphans: 2;
    widows: 2;
    text-align: left;
  }
  .c4 {
    text-decoration-skip-ink: none;
    -webkit-text-decoration-skip: none;
    color: #1155cc;
    text-decoration: underline;
  }
  .c0 {
    color: inherit;
    text-decoration: inherit;
  }
  .c9 {
    height: 11pt;
  }

  img {
    max-inline-size: 100%;
    block-size: auto;
  }

  button {
    cursor: pointer;
  }

  a {
    cursor: pointer;
  }
</style>
